...
extrafi_contract_abi = abi_data['extrafi']
extrafi_contract_address = "0xBB505c54D71E9e599cB8435b4F0cEEc05fC71cbD"  # Адрес контракта
...
# Radiant
def extrafi_deposit(wallet_address, private_key, web3, i, GAS_PRICE):
    # Инициализация контракта
    extrafi_contract = web3.eth.contract(address=web3.to_checksum_address(extrafi_contract_address),
                                         abi=extrafi_contract_abi)
    ...
    gas_limit = web3.eth.estimate_gas({
        'from': wallet_address,
        'to': extrafi_contract_address,
        'value': payable_amount,
        'data': extrafi_contract.encodeABI(fn_name='depositAndStake', args=[reserveId, amount, onBehalfOf, referralCode]),
    })

    print(f'Start with wallet [{i}/{len(wallets)}]: {wallet_address}')
    print(Fore.CYAN + f'Extrafi deposit {web3.from_wei(payable_amount, "ether")} eth')
    try:
        tx_params = {
            'nonce': nonce,
            'gasPrice': gas_price,
