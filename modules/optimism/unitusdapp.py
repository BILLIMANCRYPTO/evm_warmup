...
unitus_contract_abi = abi_data['unitus']
unitus_contract_address = "0xA7A084538DE04d808f20C785762934Dd5dA7b3B4"  # Адрес контракта
...
# Radiant
def unitus_deposit(wallet_address, private_key, web3, i, GAS_PRICE):
    # Инициализация контракта
    unitus_contract = web3.eth.contract(address=web3.to_checksum_address(unitus_contract_address),
                                         abi=unitus_contract_abi)
    ...
    gas_limit = web3.eth.estimate_gas({
        'from': wallet_address,
        'to': unitus_contract_address,
        'value': payable_amount,
        'data': unitus_contract.encodeABI(fn_name='mint', args=[recipient]),
    })

    print(f'Start with wallet [{i}/{len(wallets)}]: {wallet_address}')
    print(Fore.CYAN + f'Unitus deposit {web3.from_wei(payable_amount, "ether")} eth')
    try:
        tx_params = {
            'nonce': nonce,
            'gasPrice': gas_price,
