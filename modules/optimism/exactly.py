...
exactly_contract_abi = abi_data['exactly']
exactly_contract_address = "0x29bAbFF3eBA7B517a75109EA8fd6D1eAb4A10258"  # Адрес контракта
...
# Radiant
def exactly_deposit(wallet_address, private_key, web3, i, GAS_PRICE):
    # Инициализация контракта
    exactly_contract = web3.eth.contract(address=web3.to_checksum_address(exactly_contract_address),
                                         abi=exactly_contract_abi)
    ...
    gas_limit = web3.eth.estimate_gas({
        'from': wallet_address,
        'to': exactly_contract_address,
        'value': payable_amount,
        'data': exactly_contract.encodeABI(fn_name='deposit'),
    })

    print(f'Start with wallet [{i}/{len(wallets)}]: {wallet_address}')
    print(Fore.CYAN + f'Exactly deposit {web3.from_wei(payable_amount, "ether")} eth')
    try:
        tx_params = {
            'nonce': nonce,
            'gasPrice': gas_price,
