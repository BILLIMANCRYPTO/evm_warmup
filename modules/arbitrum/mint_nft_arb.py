...
spartanft_contract_abi = abi_data['spartanft']
spartanft_contract_address = "0x31449423737538cDe5F60C5052F122D599471Cb9"  # Адрес контракта

# Генерация кошельков из приватных ключей
wallets = [Account.from_key(private_key).address for private_key in private_keys]

# spartanft mint
def sparta_mint(wallet_address, private_key, web3, i, GAS_PRICE):
    # Инициализация контракта
    spartanft_contract = web3.eth.contract(address=web3.to_checksum_address(spartanft_contract_address), abi=spartanft_contract_abi)
    ...
    gas_limit = web3.eth.estimate_gas({
        'from': wallet_address,
        'to': spartanft_contract_address,
        'value': payable_amount,
        'data': spartanft_contract.encodeABI(fn_name='mint'),
    })

    print(f'Start with wallet [{i}/{len(wallets)}]: {wallet_address}')
    print(Fore.CYAN + f'Mint NFT')
    try:
        tx_params = {
            'nonce': nonce,
            'gasPrice': gas_price,
