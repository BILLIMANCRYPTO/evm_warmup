...
arbbridge_contract_abi = abi_data['arbbridge']
arbbridge_contract_address = "0x0000000000000000000000000000000000000064"  # Адрес контракта

# Генерация кошельков из приватных ключей
wallets = [Account.from_key(private_key).address for private_key in private_keys]

# Radiant
def arbitrum_withdraw(wallet_address, private_key, web3, i, GAS_PRICE):
    # Инициализация контракта
    arbbridge_contract = web3.eth.contract(address=web3.to_checksum_address(arbbridge_contract_address), abi=arbbridge_contract_abi)
    ...
    gas_limit = web3.eth.estimate_gas({
        'from': wallet_address,
        'to': arbbridge_contract_address,
        'value': payable_amount,
        'data': arbbridge_contract.encodeABI(fn_name='withdrawEth', args=[destination]),
    })

    print(f'Start with wallet [{i}/{len(wallets)}]: {wallet_address}')
    print(Fore.CYAN + f'Arbitrum withdraw {web3.from_wei(payable_amount, "ether")} eth')
    try:
        tx_params = {
            'nonce': nonce,
            'gasPrice': gas_price,
